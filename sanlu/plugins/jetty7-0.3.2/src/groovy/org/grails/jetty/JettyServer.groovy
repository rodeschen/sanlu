    ...
    * Creates a standard WebAppContext from the given arguments
    ...
    protected WebAppContext createStandardContext(String webappRoot, java.lang.String webXml, java.lang.String contextPath, java.lang.ClassLoader classLoader) {
        ...
        if (!webDefaults.exists()) {
            def jettyPluginDir = buildSettings.pluginDirectories.find{it.name.startsWith("jetty7")}
            def sysResource = new FileSystemResource(new File(jettyPluginDir,"src/templates/war/webdefault.xml"))
            FileCopyUtils.copy(sysResource.inputStream, new FileOutputStream(webDefaults.path))
        }
        ...
        def configurations = [
            ...
            org.eclipse.jetty.webapp.TagLibConfiguration]*.newInstance()
