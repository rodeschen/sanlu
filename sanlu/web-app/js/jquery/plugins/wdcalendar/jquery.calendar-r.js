/**
 * @description {Class} wdCalendar This is the main class of wdCalendar.
 */
;
(function($) {
	...
	$.fn.bcalendar = function(option) {
		var def = {
			/**
			 * @description {Config} view {String} Three calendar view provided,
			 *              'day','week','month'. 'week' by default.
			 */
			view : "week",
			/**
			 * @description {Config} weekstartday {Number} First day of week 0
			 *              for Sun, 1 for Mon, 2 for Tue.
			 */
			weekstartday : 1, // start from Monday by default
			theme : 0, // theme no
			/**
			 * @description {Config} height {Number} Calendar height, false for
			 *              page height by default.
			 */
			height : false,
			/**
			 * @description {Config} url {String} Url to request calendar data.
			 */
			url : "",
			/**
			 * @description {Config} eventItems {Array} event items for
			 *              initialization.
			 */
			eventItems : [],
			method : "POST",
			/**
			 * @description {Config} showday {Date} Current date. today by
			 *              default.
			 */
			showday : new Date(),
			/**
			 * @description {Event} onBeforeRequestData:function(stage) Fired
			 *              before any ajax request is sent.
			 * @param {Number}
			 *            stage. 1 for retrieving events, 2 - adding event, 3 -
			 *            removiing event, 4 - update event.
			 */
			onBeforeRequestData : false,
			/**
			 * @description {Event} onAfterRequestData:function(stage) Fired
			 *              before any ajax request is finished.
			 * @param {Number}
			 *            stage. 1 for retrieving events, 2 - adding event, 3 -
			 *            removiing event, 4 - update event.
			 */
			...
			/**
			 * @description {Event} onAfterRequestData:function(stage) Fired
			 *              when some errors occur while any ajax request is
			 *              finished.
			 * @param {Number}
			 *            stage. 1 for retrieving events, 2 - adding event, 3 -
			 *            removiing event, 4 - update event.
			 ...
			 *              Fired when user quick adds an item. If this function
			 *              is set, ajax request to quickAddUrl will abort.
			 * @param {Object}
			 *            calendar Calendar object.
			 * @param {Array}
			 *            param Format [{name:"name1", value:"value1"}, ...]
			 * 
			 */
			quickAddHandler : false,
			/**
			 * @description {Config} quickAddUrl {String} Url for quick adding.
			 */
			quickAddUrl : "",
			/**
			 * @description {Config} quickUpdateUrl {String} Url for time span
			 *              update.
			 */
			quickUpdateUrl : "",
			/**
			 * @description {Config} quickDeleteUrl {String} Url for removing an
			 *              event.
			 */
			quickDeleteUrl : "",
			/**
			 * @description {Config} autoload {Boolean} If event items is empty,
			 *              and this param is set to true. Event will be
			 *              retrieved by ajax call right after calendar is
			 *              initialized.
			 */
			autoload : false,
			/**
			 * @description {Config} readonly {Boolean} Indicate calendar is
		...
		function render() {
			...
			if (option.view == "day" || option.view == "week") {
				var $dvtec = $("#dvtec");
				if ($dvtec.length > 0) {
					option.scoll = $dvtec.attr("scrollTop"); // get scroll
					// bar position
				}
			}
			switch (option.view) {
		...
		function PropareEvents(dayarrs, events, aDE, sDE) {
			...
			for ( var i = 0; i < l; i++) { // to deal with more-than-one-day
				// event
				var de = deB[i];
				if (de.length > 0) { //           
			...
			for ( var i = 0; i < l; i++) {
				...
				if (de.length > 0) {
					...
					for ( var y = 0; y < t; y++) {
						...
						x.aQ = Math.min(1 - x.left, x.width + 0.7 * p); // width
						// offset
					}
					de = null;
					deA[i] = k;
			...
			if (option.url && option.url != "") {
				...
				$
						.ajax({
							...
							// Ms Date Json Format /Date(@Tickets)/
							dataType : "json",
		...
		function parseED(data) {
			if (data.length > 6) {
				...
				e.push(data[0], data[1], new Date(data[2]), new Date(data[3]),
						...
						data[8] != undefined ? parseInt(data[8]) : 0, data[9],
						data[10]);
				return e;
			}
			return null;
		...
		function getbuddlepos(x, y, obj) {

			var tleft = x - (obj && obj.width() || 110);
			var ttop = y - (obj && obj.height() || 217);
		...
		function quickadd(start, end, isallday, pos) {
			...
			if (buddle.length == 0) {
				var temparr = [];
				temparr
						.push('<div id="bbit-cal-buddle" style="z-index: 180; width: 600px;visibility:hidden;" class="bubble">');
				temparr
				...
				temparr
						.push(
								i18n.xgcalendar.update_detail,
								'</div></div></div><tr><td><div id="bl1" class="bubble-corner"><div class="bubble-sprite bubble-bl"></div></div><td><div class="bubble-bottom"></div><td><div id="br1" class="bubble-corner"><div class="bubble-sprite bubble-br"></div></div></tr></tbody></table><div id="bubbleClose1" class="bubble-closebutton"></div><div id="prong2" class="prong"><div class=bubble-sprite></div></div></div>');
				var tempquickAddHanler = temparr.join("");
				temparr = null;
				var tmp = $(tempquickAddHanler).appendTo($(document.body));
				tmp.find(".cb-root").append(
						$("#ckbub").find("#cltabs").tabs().end());
				buddle = $("#bbit-cal-buddle");
				var calbutton = $("#caladd");
				var __startDate = {}, __endDate = {};
				...
				calbutton
						.click(function(e) {
							...
							var zone = new Date().getTimezoneOffset() / 60 * -1;
							buddle.find("#cal-start-year")
									.val(__startDate.year);
							buddle.find("#cal-start-month").val(
									__startDate.month);
							buddle.find("#cal-start-day").val(__startDate.day);
							buddle.find("#cal-start-hour")
									.val(__startDate.hour);
							buddle.find("#cal-start-min").val(__startDate.min);

							buddle.find("#cal-end-year").val(__endDate.year);
							buddle.find("#cal-end-month").val(__endDate.month);
							buddle.find("#cal-end-day").val(__endDate.day);

							buddle.find("#cal-end-hour").val(__endDate.hour);
							buddle.find("#cal-end-min").val(__endDate.min);
							var param = {
								"type" : what,
								"startTime" : buddle.find("#cal-start-year")
										.val()
										+ "-"
										+ buddle.find("#cal-start-month").val()
										+ "-"
										+ buddle.find("#cal-start-day").val()
										+ " "
								...
								"endTime" : buddle.find("#cal-end-year").val()
										...
										+ "-"
										+ buddle.find("#cal-end-day").val()
										+ " "
										+ buddle.find("#cal-end-hour").val()
										+ ":"
										+ buddle.find("#cal-end-min").val(),
								"isAllDay" : allday,
								"timezone" : zone
							}
							// 修改參數方式
							$.extend(param, option.extParam || {});
							$("#bbit-cal-buddle").css("visibility", "hidden");
							var newdata = [];
							var tId = -1;
			...
			var off = getbuddlepos(pos.left, pos.top, buddle);
			if (off.hide) {
				$("#prong2").hide()
			} else {
				$("#prong2").show()
			...
			$("#bbit-cal-start")
					.val(
							(sd = dateFormat.call(start,
									i18n.xgcalendar.dateformat.fulldayvalue
											+ " HH:mm")));
			$("#bbit-cal-end")
					.val(
							(ed = dateFormat.call(end,
									i18n.xgcalendar.dateformat.fulldayvalue
											+ " HH:mm")));
			__startDate = converDateToJSON(sd);
			__endDate = converDateToJSON(ed);
			buddle.find("#cal-start-year").val(__startDate.year);
			buddle.find("#cal-start-month").val(__startDate.month);
			buddle.find("#cal-start-day").val(__startDate.day);
			buddle.find("#cal-start-hour").val(__startDate.hour);
			buddle.find("#cal-start-min").val(__startDate.min);
		...
		function converDateToJSON(date) {
			var object = {}
			object.year = date.substr(0, 4);
			object.month = date.match(/-[0-9]{1,2}-/)[0].replace(/-/g, "");
			object.day = date.match(/-[0-9]{1,2} /)[0].replace(/[- ]/g, "");
			object.hour = parseInt(date.match(/ [0-9]{2}:/)[0].replace(/[: ]/g,
					""));
			object.min = parseInt(date.match(/:[0-9]{1,2}$/)[0].replace(/[:]/g,
					""));
			return object;
		}
		...
		var c = {
			...
			nt : function() {
				...
				case "month":
					...
					{
						option.showday = DateAdd("d", 0 - nd, option.showday); // last
						// day
						// of
						// last
						// month
	 ...
	 * @param {String}
	 *            view View name, one of 'day', 'week', 'month'.
	 */
	...
	/**
	 * @description {Method} gotoDate To go to a range containing date. If view
	 *              is week, it will go to a week containing date. If view is
	 *              month, it will got to a month containing date.
	 * @param {Date}
	 *            date. Date to go.
	 */
	...
	/**
	 * @description {Method} previousRange To go to previous date range. If view
	 *              is week, it will go to previous week. If view is month, it
	 *              will got to previous month.
	 */
	...
	/**
	 * @description {Method} nextRange To go to next date range. If view is
	 *              week, it will go to next week. If view is month, it will got
	 *              to next month.
	 */
